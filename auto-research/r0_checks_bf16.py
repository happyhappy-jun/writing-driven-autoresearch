...
def main():
    ...
    ref = AutoModelForCausalLM.from_pretrained(MODEL, torch_dtype=torch.bfloat16).to(DEV).eval()
    wrp = AutoModelForCausalLM.from_pretrained(MODEL, torch_dtype=torch.bfloat16).to(DEV).eval()
    dm = DepthARModel(wrp)
    L = dm.n_layers
    print(f"model: {MODEL}  layers={L}  dtype=bf16  eligible={dm.eligible_layers()}", flush=True)
    ...
    with torch.no_grad():
        ...
        # fp32 round-off in the reconstruction h + (out - h); the boundary itself is exact.
        check("4_boundary_capture", worst <= 1e-4 * scale,
    ...
    green = n_pass == 6
    payload = {
        "run_id": "r0_checks_0.5b_bf16", "round": 0, "model": MODEL,
        "dtype": "bfloat16", "device": "RTX 3090 (Ampere, sm_86) alin14", "n_layers": L,
        "eligible_layers": dm.eligible_layers(),
        "checks": results, "n_pass": n_pass, "n_total": 6,
        "status": "complete" if green else "failed",
        "notes": "BF16 on sm_86; dense/alpha0/manual-skip checks are bit-exact.",
    }
    p = write_result(os.path.expanduser("~/depthar_results/r0_checks_0.5b_bf16.json"), payload)
